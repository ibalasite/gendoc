...
| Error Code | HTTP Status | 觸發條件 | Response Body 範例 |
|-----------|-------------|---------|-------------------|
| `VALIDATION_ERROR` | 400 | `email` 格式不符合 RFC 5322 | `{"code":"VALIDATION_ERROR","errors":[{"field":"email","message":"Invalid format"}]}` |
| `CONFLICT` | 409 | 該 email 已被其他帳號使用 | `{"code":"CONFLICT","message":"Email already registered"}` |
| `RATE_LIMITED` | 429 | 同一 IP 每分鐘超過 10 次請求 | `{"code":"RATE_LIMITED","retry_after":60}` |
```
