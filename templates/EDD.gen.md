...
- §1 概述（一段話說明技術方案）
- §2 技術選型
- §3 Clean Code 架構（SOLID 原則對應表 + 分層設計）
- §3.4 Bounded Context & Context Map
- §4 Security 設計（OWASP Top 10 + Secret 管理）
- §4.5 Domain Events
- §5 BDD 設計（Gherkin Scenario）
- §5.5 資料模型（Entity 清單 + ER 圖）
- §6 TDD 設計（測試金字塔）
- §7 SCALE 設計（規格推算 + Load Test + k8s 資源）
- §8 可觀測性設計
...
### §10 Mermaid 架構圖（9 種 UML）（強制，所有 9 種圖每種至少一張，缺一不可）

> **Iron Law**：任何 EDD 必須在 §10 完整包含以下 9 種 UML 圖，每種至少一張。
> 不得留空、不得用文字替代、設計評審前必須完成。
> 
> **重要**：UML 圖集放在 §10（Mermaid 架構圖），不是 §4.5（Domain Events）。
> gendoc-gen-diagrams 從 §10 提取所有圖。

必須包含的 UML 圖種類（§10.1–§10.9）：

1. **§10.1 Use Case Diagram（使用案例圖）**：每個主要 Actor 角色的使用案例（Mermaid flowchart TD 或 graph TD）
2. **§10.2 Class Diagram（類別圖）⭐（最重要）**：含 Interface 定義，依架構層次分張（Domain/Application/Infrastructure/Presentation 各一）；每個 class 必須標注 stereotype（<<Entity>>/<<UseCase>>/<<Repository>> 等），並列出所有 public method 含回傳型別
3. **§10.3 Object Diagram（物件圖）**：具體物件實例快照，欄位含真實範例值（非型別定義）
4. **§10.4 Sequence Diagram（循序圖）**：每個主要業務流程各一張（Happy Path + Error Path 分開）；參與者含 Controller/Service/Repository/DB
5. **§10.5 Communication Diagram（通訊圖）**：元件間通訊訊息序列，訊息標注序號（Mermaid flowchart LR）
6. **§10.6 State Machine Diagram（狀態機圖）**：每個有狀態 Entity（Order/User/Payment 等）各一張（Mermaid stateDiagram-v2）
7. **§10.7 Activity Diagram（活動圖）**：每個關鍵業務流程各一張，含決策點和平行路徑（Mermaid flowchart TD）
8. **§10.8 Component Diagram（元件圖）**：系統架構元件依賴（Mermaid flowchart LR）
9. **§10.9 Deployment Diagram（部署圖）**：k8s 部署拓撲（Mermaid flowchart TD）
...
每個 classDiagram 生成後，必須在 §10.2 尾部加入 Class Inventory 表格：
...
- [ ] §7 SCALE 有具體數字（非「視情況」）
- [ ] §10 UML 9 大圖全部完成（§10.1 Use Case / §10.2 Class / §10.3 Object / §10.4 Sequence / §10.5 Communication / §10.6 State / §10.7 Activity / §10.8 Component / §10.9 Deployment 每種至少一張）
- [ ] Sequence 含 Happy Path + Error Path + 每個主要流程各一張
- [ ] Class 依層次分張（Domain/Application/Infrastructure 各一或 Client/Server 各一）
- [ ] Use Case 含每個主要角色
- [ ] §4 OWASP Top 10 每項都有具體對策
- [ ] §3.4 Bounded Context Map（若有多服務）已繪製
- [ ] §4.5 Domain Events 清單已完整列出（注意：Domain Events 在 §4.5，UML 圖在 §10，兩者不同）
