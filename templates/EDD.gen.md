...
quality-bar: "所有 PRD P0 功能有對應模組；§7 SCALE 有具體數字；UML 9 大圖全部完成（每個 API endpoint → Sequence Diagram；每個 DB 資料表 → ERD；每個 PRD AC → 流程圖或 Sequence Diagram）；OWASP Top 10 每項有具體對策；STRIDE 6 威脅類型均有對策；SLO/SLI 已量化；Capacity Planning 含計算公式；§20 Feature Flag 5 種類型均已評估；§21 觀測性三支柱均已設計；Runbook 框架已設計；所有數字明確，不得留「視情況」佔位。"
---
...
| 數值非 TBD/N/A | 所有效能預算、API 回應時間、記憶體限制填有實際數字 | 從 PRD §Success Metrics 推算填入 |
