...
若 `docs/EDD.md` 不存在，`entity_count` 使用預設值 0；`rest_endpoint_count` 因 Step 2-B 的 fallback 邏輯（<5 時強制設為 10）將使用預設值 10，並在 MANIFEST.md 的對應欄位標注 `（EDD 尚未生成，請重新執行 DRYRUN）`。

若讀取後 `docs/EDD.md` 不存在或 entity_count=0 且 rest_endpoint_count 使用 fallback=10（兩者同時為空），在 MANIFEST.md §2.1 的對應欄位標注 `（WARNING：EDD 尚未生成或內容不足，請在 EDD 生成後重新執行 DRYRUN）`。

> **設計說明**：DRYRUN 不執行上游衝突偵測（各文件間語義矛盾的判斷）。若上游文件之間存在矛盾，應由各自的 review 步驟處理；DRYRUN 只計算數字，不判斷文件語義一致性。
