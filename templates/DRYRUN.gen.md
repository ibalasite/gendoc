...
upstream-docs:
  dynamically-determined: templates/pipeline.json metrics[] array
  description: |
    Instead of hardcoding specific files (EDD, PRD, ARCH), upstream files are
    determined dynamically from pipeline.json metrics[].each.source_step.
    This ensures new metrics added to pipeline.json are automatically used.
  always-required:
    - templates/pipeline.json    # Contains metrics[] definitions (SSOT)
...
## Step 1：動態讀取上游文件（根據 pipeline.json 的 metrics 定義）

**SSOT 原則**：不「特別列出」EDD/PRD/ARCH，而是根據 `templates/pipeline.json` 的 `metrics[]` 陣列動態決定讀取哪些檔案。

### 1-A. 讀取 pipeline.json

```bash
# 加載完整的 metrics 定義
_METRICS_DEFS=$(python3 -c "
import json
with open('templates/pipeline.json') as f:
    defs = json.load(f)
    for metric in defs.get('metrics', []):
        source = metric.get('source_step', 'UNKNOWN')
        pattern = metric.get('grep_pattern', '')
        fallback = metric.get('fallback', 0)
        print(f\"{metric['id']},{source},{pattern},{fallback}\")
")
```

### 1-B. 遍歷並動態讀取每個 source_step

```bash
# 根據 metrics[] 定義，動態讀取每個指定的 source_step 文件
# 無硬編碼的 EDD、PRD、ARCH 列表，完全由 pipeline.json 驅動
while IFS=',' read -r METRIC_ID SOURCE_STEP PATTERN FALLBACK; do
  SOURCE_FILE="docs/${SOURCE_STEP}.md"
  if [[ -f "$SOURCE_FILE" ]]; then
    COUNT=$(grep -c "$PATTERN" "$SOURCE_FILE" 2>/dev/null || echo "$FALLBACK")
    echo "${METRIC_ID}=${COUNT}"
  else
    echo "${METRIC_ID}=${FALLBACK} (missing: $SOURCE_FILE)"
  fi
done <<< "$_METRICS_DEFS"
```
...
> **設計說明**：
> - DRYRUN 不執行上游衝突偵測（各文件間語義矛盾）。若存在矛盾，由各自的 review 步驟處理。
> - DRYRUN 只計算數字，不判斷語義一致性。
> - **新增 metric 無需改 dryrun.gen.md**：只需在 pipeline.json metrics[] 新增一項，本步驟自動讀取。

---

## Step 2：計算量化錨點（Reference Implementation）

**注意**：Step 1-B 已透過 pipeline.json metrics[] 的動態迴圈完成量化計算。以下小節是 **參考實現** — 說明每個 metric 的 grep_pattern 如何運作。

若使用 dryrun_core.py（推薦），無需手動執行以下 bash 命令；Python 實現已自動完成。

### Legacy Bash Reference（僅供參考）
