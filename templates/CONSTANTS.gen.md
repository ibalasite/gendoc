...
quality-bar: "§1-§8 所有章節均有實際數值（無 placeholder）；每個數值均有 PRD §來源標注；constants.json 同步生成且與 .md 一致；全文無任何 {{...}} 佔位符；數值與 PRD 對照無誤差；§4 SLO 三項（可用性/P99 延遲/錯誤率）均有量化數值；§8 容量三項（並發用戶/峰值 TPS/資料保留）均有量化數值；§7 業務規則已填入或明確標記「本 PRD 無額外業務規則數值」。"
---
...
- 發現衝突時標記 `[CONFLICT: PRD §X 值=A vs PRD §Y 值=B]`，提取較保守的數值作為常數
...
- 開放性掃描 PRD 中未被 §1-§6 涵蓋的業務規則量化數值
- 彈性填寫：項目數量視 PRD 內容而定，不限筆數
- 若 PRD 確實無此類量化業務規則，明確標記「本 PRD 無額外業務規則數值」，不得保留 placeholder
...
| 項目 | 填寫來源 |
|------|---------|
| 最大並發用戶 | PRD §非功能需求 / BRD §業務規模 |
| 峰值 TPS | PRD §非功能需求 / BRD §業務規模 |
| 資料保留期限 | PRD §資料政策 / BRD §合規要求 |

若 PRD 未定義上述任一項，必須標記 `TBD（PRD 未定義，需 PM 確認）`，不得留空或保留 placeholder。

### Step 10：生成 docs/constants.json
...
| §7 業務規則或明確標記「本 PRD 無額外業務規則數值」 | 確認 §7 無 placeholder；若無業務規則數值，確認已標記「本 PRD 無額外業務規則數值」 | 重新掃描 PRD 補入，或補上明確標記 |
| §8 容量三項均有值 | 確認並發/TPS/保留期限非空 | 標記 TBD 並說明 |
| constants.json 存在 | `ls docs/constants.json` | 重新生成 |
| JSON 與 .md 數值一致 | 抽樣對照 5 個數值 | 以 .md 為準修正 constants.json；若 .md 數值與 PRD 亦不符，先修正 .md（回到 PRD 來源），再重新生成 constants.json |
