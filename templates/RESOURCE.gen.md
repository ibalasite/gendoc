...
quality-bar: "VDD §4 所有角色均有 RES-IMG 行；ANIM.md 所有 SKEL/VFX 均有 RES-ANIM 行（若存在）；AUDIO.md 所有 BGM/SFX 均有 RES-BGM/RES-SFX 行（若存在）；所有 file_size_budget 非空白且符合平台預算；status=needed 以外的資產 prompt 非空白；無裸 placeholder"
---
...
2. 對每個角色生成至少一個 RES-IMG 行：
   ...
   - `type`：`image`
   - `source_tool`：依 VDD 的視覺風格自動推薦（2D 卡通 → Midjourney v6；像素風 → Stable Diffusion；3D 寫實 → Midjourney v6 或 DALL-E 3）；留空讓人工確認填寫
...
2. 對每個 SKEL-xxx 生成一個 RES-ANIM 行：
   ...
   - `type`：`animation`
   - `source_tool`：依引擎選型（Unity → Spine（`.skel`）或 Unity Animator；Cocos → DragonBones（`.json`）或 Spine；Web → Lottie / CSS Animation）
