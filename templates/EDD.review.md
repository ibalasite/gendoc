...
**Fix**: 補跑 `gendoc-gen-diagrams` skill，會自動從 EDD 提取並生成全部 9 種 UML 圖。多圖原則：每個主要業務流程一張 Sequence Diagram（≥ 3 張），每個有狀態 Entity 一張 State Machine，每個 P0 User Story 一張 Activity Diagram（≥ 3 張）。
...
**Fix**: 補跑 `gendoc-gen-diagrams` skill，`class-inventory.md` 由工具自動從三張 Server Class Diagrams 提取；若需人工補充，表格欄位為：Class 名稱 / Stereotype / 架構層 / 推斷 src 路徑 / 推斷 test 路徑，必須覆蓋所有出現在 classDiagram 中的 class。
