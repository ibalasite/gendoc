...
| 檢查項 | 合格標準 | 不合格處理 |
|--------|---------|-----------|
| 所有 §章節齊全 | 對照 test-plan.md 章節清單，無缺失章節 | 補寫缺失章節 |
| 無裸 placeholder | 每個 `{{...}}` 後有「: 說明」或具體範例值 | 補全說明或替換為具體值 |
| 技術棧一致 | 測試框架與 state.lang_stack 一致（Jest/pytest/RSpec/Go test 等） | 修正至一致 |
| TC 步驟具體 | 每個 TC 的測試步驟非「點擊按鈕」，而是具體動作 + 具體預期結果 | 補充具體步驟和預期結果 |
