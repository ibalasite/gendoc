...
6. **LOCAL_DEPLOY.md**：依文件 step-by-step 指令，AI 或人類可在 local 完整建置 k8s 環境（以 Rancher Desktop 實現）；**client 也封進 k8s pod**，所有服務均在 k8s 內；**對外只有一個 port**，測試入口單一；同時提供 docker-compose 版本作為輔助（兩種方式均可無礙建置，不能有缺漏步驟或 TBD）
