...
6. **LOCAL_DEPLOY.md**：依文件 step-by-step 指令，AI 或人類可在 local 完整建置 k8s 環境（以 Rancher Desktop 實現）；**client 也封進 k8s pod**，所有服務均在 k8s 內；**對外只有一個 port（Port 80）**，測試入口單一；同時提供 docker-compose 版本作為輔助（兩種方式均可無礙建置，不能有缺漏步驟或 TBD）；**Local Developer Platform 完整可用**：Gitea（Port 3000，本地 git server）+ Jenkins（Port 8080，CI）+ ArgoCD（Port 8443，CD GitOps）均以 k3s pod 運行，透過 `make dev-tools-up` 一鍵啟動；開發工具 port 與應用 port 80 域分離、不衝突；**非開發者也能操作**：`make gitea-ui` / `make jenkins-ui` / `make argocd-ui` 提供 UI 入口，不需懂 k8s；**Production Parity**：本地與生產使用完全相同工具鏈（Jenkins + ArgoCD + k8s），差異只在規模與 TLS
...
graph TD
    ...
    AUDIT["稽核層<br/>D16 ALIGN_REPORT"]
    IMPL["實作層<br/>D17 docs/blueprint/contracts/ infra/ scaffold/<br/>D18 docs/blueprint/mock/"]
    PUB["發布層<br/>D19 docs/pages/"]
