...
## 7.8 special_skill 步驟 Stale 自動重建規則

### 背景：兩類步驟的不同重建語意

gendoc pipeline 中的步驟分為兩大類，對「上游文件被更新」的處理方式根本不同：
...
**設計原則**：AI gen 步驟的「不重跑」是為了保留業務決策；special_skill 步驟的「必重跑」是因為其輸出是上游的確定性投影，過期即錯誤。

---

### 演算法：Makefile mtime 語意

```
for each step in pipeline.json where step.special_skill is defined:
    trigger_files = step.input[]
    if trigger_files is empty:
        trigger_files = glob("docs/*.md")   # HTML 步驟的隱式觸發集

    newest_input  = max(mtime(f) for f in trigger_files)
    oldest_output = min(mtime(f) for f in expand(step.output))

    if newest_input > oldest_output:
        status = STALE   → rebuild
    else:
        status = FRESH   → skip
```

**無硬編碼**：觸發集（`input`）、輸出基準（`output`）、以及是否啟用 mtime 檢查（`special_skill` 存在）全部從 `pipeline.json` 動態讀取。新增或移除步驟只需修改 `pipeline.json`，演算法自動適應。
...
| 欄位 | 型別 | 用途 |
|------|------|------|
| `special_skill` | string（技能名稱） | 存在 → 啟用 mtime 檢查；值即為執行該步驟時呼叫的 skill |
| `input` | string[]（相對路徑） | 觸發集；空陣列 = 隱式觸發集（所有 `docs/*.md`） |
| `output` | string 或 string[]（相對路徑或 glob） | 輸出基準；取所有匹配檔案的最舊 mtime |

不需新增欄位，現有三個欄位已完整描述重建邏輯。
...
1. **自動執行，無需使用者確認**：mtime 比對是機械判斷，不含業務決策，不詢問。
2. **重建順序遵循 pipeline 順序**：若多個 special_skill 步驟同時 STALE，依 `pipeline.json` 的步驟順序依序重建（上游先、下游後）。
3. **重建完成後不重跑 AI gen 步驟**：例如 HTML STALE 被重建後，不會回頭觸發 API 或 SCHEMA 的 review loop。
4. **FRESH 步驟不重建**：mtime 確認新鮮的步驟直接跳過，不論 `completed_steps` 記錄。
