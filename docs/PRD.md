...
<!-- Version: v4.4 | Status: ACTIVE | DOC-ID: PRD-GENDOC-20260422 -->
...
| 版本 | 日期 | 作者 | 變更摘要 |
|------|------|------|---------|
| v4.4 | 2026-05-13 | PM Agent | **gen_html.py 視覺/排版能力 A→W 群 TDD GREEN 完整收斂 + Windows cp950 修復 + setup per-package build**：(1) **gen_html.py 大規模 TDD 重構（131 commits / ~67 個 gen-html 相關，~4k LOC）**：完整覆蓋 A→W 字母群階段，每群皆 RED→GREEN 雙 commit 驗證 — A 群（table padding/hover、input border、card/modal title 底線）/ B 群（scanner）/ F 群（classifier）/ G 群（lightbox + PUML）/ I 群（template — SCHEMA/EDD 雙段式「說明 table 先、語法後」）/ K 群（umock parser + flush_text + ASCII 對齊 + Prism 干擾修正 + assets-deploy 自動 sync canonical）/ L 群（subdir HTML 通用 root-prefix path-rewriter — CSS/nav-brand/breadcrumb）/ M 群（prototype docs link 回送 named tab）/ N 群（每頁 sidebar TOC tab + scroll-spy + 收合 + RWD，13/13 GREEN）/ P 群（mermaid edge label quote 不二次破壞 + pyramid SVG width/height）/ Q 群（header 兩層等高 56px + 白條消除）/ R 群（sidebar 文件 label 移除 + 全展開無 scrollbar + 留白縮減）/ S 群（HTML comment strip + metadata `---` separator strip + 空白 collapse）/ T 群（sidebar sticky + tabs anchored + panel internal scroll）/ U 群（top-nav breadcrumb 當前頁名 smooth-scroll 回頁首）/ V 群（TOC / sidebar anchor jump 避開 sticky top-nav）/ W 群（title-block H1+H2 subtitle 後多餘 hr strip）；另含 UI Mock DSL 11-stage 完整管線（parser→renderer→layered-arch→pyramid→ASCII frame/table/form-row/inner-boxes→md→HTML 整合→10 個 pet/erp 真實 mockup 視覺驗證）+ mermaid `<br/>` / `[guard]` bracket emit-time 修正 + 空 state block 移除 + lightbox class 對齊 app.js DOM + visual-lock baseline（256+3 mermaid+umock fixtures）；(2) **Windows cp950 UTF-8 修復（commit `cc4a649`）**：`setup.ps1` 開頭注入 `PYTHONIOENCODING=utf-8` + `PYTHONUTF8=1` + `[Console]::OutputEncoding=UTF8`；`bin/gendoc-settings-hook.py` 開頭 `sys.stdout.reconfigure(encoding="utf-8")`；解決 zh-TW Windows install 時 hook 註冊輸出 `✅ ... 已加入` 觸發 `UnicodeEncodeError` 中斷的問題，防禦縱深兩層保證任何呼叫路徑都安全；(3) **setup per-package build（commit `307c43c`）**：`_deploy_tools` 偵測 `tools/<pkg>/build.ps1` 或 `build.sh` 並以 `BIN_DIR` / `PACKAGE_DIR` env 呼叫，由 package 自帶 build；無 build 腳本時 fallback 為 `cp <pkg>.py → bin/<pkg>.py`；讓 packages 可演化成 multi-file 結構而不需改 setup；(4) **gendoc-guard 補強**：Stop-hook dodge detection（commit `8cbf6ad` — 列舉中含 skip-intent 詞即擋）+ checkpoint exemption（commit `c04f58b` — wrapper Step 3 自清不被自擋）+ R-13 攔截 `git commit --allow-empty`（防 touch + empty commit 繞品管）；(5) **影響範圍**：純工具層 / 文件呈現層 / Windows 平台支援；不動 pipeline 結構、不改 DRYRUN spec、不改 skill 邊界；§7.10 DRYRUN 重構、§7.11 guard priming 訊息層仍依 v4.2 / v4.3 規格執行中。 |
...
## 7.8 Branch B 三層驗證架構（L1 / L2 / L3）

### 背景：兩類步驟的不同重建語意

gendoc pipeline 中的步驟分為兩大類，對「輸出需要重建」的判斷邏輯不同：

| 類型 | 代表步驟 | 上游更新後的行為 | 理由 |
|------|---------|----------------|------|
| **AI gen 步驟** | PRD、EDD、ARCH、API、SCHEMA … | **外科修補**（review → fix loop，只補缺失章節） | AI 生成的文件包含業務判斷，不應因上游格式異動而全部重寫；review.md 的「最小修改原則」保護已有內容 |
| **special_skill 步驟** | HTML、UML、ALIGN、CONTRACTS、MOCK、PROTOTYPE … | **完整重建**（mtime-based，Makefile 語意） | 這些步驟是**純轉換函式**：輸入 → 確定性輸出，不含 AI 業務判斷；上游文件若比輸出新，輸出一定過期 |

---

### Branch B 三層驗證結構

Branch B 對每個 post-DRYRUN step 執行三層短路驗證（由淺入深，前層 FAIL 立即補跑，不繼續後層）：

```
for each step in post-DRYRUN steps:

    # L1：mtime Stale（僅 special_skill 步驟）
    if step.special_skill exists:
        newest_input  = max(mtime(trigger_files))    # trigger_files 空 → 隱式展開 docs/*.md
        oldest_output = min(mtime(output_files))
        if oldest_output is None OR newest_input > oldest_output:
            status = STALE → 短路補跑（不檢查 L2/L3）
        else:
            status = FRESH → 繼續 L2
    # 非 special_skill 步驟：跳過 L1，直接到 L2

    # L2：輸出完整性（所有步驟）
    if output files missing or empty:
        L2 FAIL → 補跑（不檢查 L3）

    # L3：量化品質門檻（所有步驟）
    read .gendoc-rules/{step_id}-rules.json
    if rules.json missing → L3 PASS（不阻擋）
    if threshold not met → L3 FAIL → 補跑
```

**設計原則**：
- L1 比 L2 先跑：若輸入比輸出新，輸出即使完整也必須重建（Makefile 語意）
- L1 STALE 且輸出存在：短路重建，避免「完整但過期」的輸出被誤判為合格
- L1 STALE 且輸出不存在：同樣走補跑路徑（L2 的輸出不存在判斷不需重複）
- Phase C 已廢除：不再有獨立的 mtime 相位，AI 無法跳過
...
| 欄位 | 型別 | 用途 |
|------|------|------|
| `special_skill` | string（技能名稱） | 存在 → 啟用 L1 mtime 檢查；值即為執行該步驟時呼叫的 skill |
| `input` | string[]（相對路徑） | L1 觸發集；空陣列 = 隱式觸發集（所有 `docs/*.md`） |
| `output` | string 或 string[]（相對路徑或 glob） | L1 輸出基準（取最舊 mtime）+ L2 存在性基準 |

---

### 受影響步驟一覽（參考，以 pipeline.json 為準）

| Step ID | special_skill | 觸發集（`input`） | 輸出基準（`output`） |
|---------|--------------|-----------------|-------------------|
| UML | `gendoc-gen-diagrams` | `docs/EDD.md`, `docs/ARCH.md`, `docs/FRONTEND.md` | `docs/diagrams/*.md` |
| UML-CICD | `gendoc-gen-diagrams` | `docs/EDD.md`, `docs/ARCH.md` | `docs/diagrams/server-cicd-pipeline.md` |
| ALIGN | `gendoc-align-check` | `docs/*.md`（實務上讀所有設計文件） | `docs/ALIGN_REPORT.md` |
| ALIGN-FIX | `gendoc-align-fix` | `docs/ALIGN_REPORT.md` | `docs/*.md`（依報告內容修改） |
| ALIGN-VERIFY | `gendoc-align-check` | `docs/*.md` | `docs/ALIGN_REPORT.md` |
| CONTRACTS | `gendoc-gen-contracts` | `docs/API.md`, `docs/SCHEMA.md`, `docs/EDD.md` | `docs/blueprint/contracts/` |
| MOCK | `gendoc-gen-mock` | `docs/API.md`, `docs/SCHEMA.md` | `docs/blueprint/mock/` |
| PROTOTYPE | `gendoc-gen-prototype` | `docs/PDD.md`, `docs/FRONTEND.md`, `docs/API.md` | `docs/pages/prototype/` |
| HTML | `gendoc-gen-html` | `[]`（隱式：所有 `docs/*.md`） | `docs/pages/*.html` |

> 此表僅供快速索引；實際觸發集與輸出基準以 `templates/pipeline.json` 的 `input` / `output` 欄位為準。

---

### 重建執行規則

1. **自動執行，無需使用者確認**：L1 mtime 比對是機械判斷，不含業務決策，不詢問。
2. **重建順序遵循 pipeline 順序**：若多個步驟同時 FAIL，依 `pipeline.json` 的步驟順序依序補跑（上游先、下游後）。
3. **重建完成後不重跑 AI gen 步驟**：例如 HTML STALE 被重建後，不會回頭觸發 API 或 SCHEMA 的 review loop。
4. **FRESH 且 L2/L3 通過的步驟直接跳過**，不論 `completed_steps` 記錄。
