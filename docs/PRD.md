...
gendoc pipeline 分為兩個相位（Phase），中間由 DRYRUN 作為閘門：

| 相位 | 步驟 | 特徵 | 品質驗收 |
|------|------|------|---------|
| **Phase A（內容相）** | IDEA → BRD → PRD → CONSTANTS → PDD → VDD → EDD → ARCH | 完全由 AI 從上游文件推導，無量化基線 | 文件自身完整性 + 跨文件上下文一致 |
| **DRYRUN 閘門** | 讀取 Phase A 全部 8 份文件，計算 20 個量化指標，推導 31 個後續 step 的規格 | 量化基線計算、規格推導、狀態存儲 | — |
| **Phase B（技術相）** | API → SCHEMA → FRONTEND → ... → HTML（23 個 step） | 依 DRYRUN 推導的規格進行品質驗收（雙層檢查：AI + Shell Script） | **AI Finding + Shell Finding 合併驗收** |

### DRYRUN 職責清單（完全 SSOT 架構）

#### Single Source of Truth 原則

DRYRUN 引擎遵循**完全 SSOT**（Single Source of Truth）架構：
- **量化指標定義** = `templates/pipeline.json` 的 `metrics[]` 陣列
- **規格推導規則** = `templates/pipeline.json` 的 `steps[].spec_rules` 欄位
- **DRYRUN 代碼** = 通用的讀取 + 計算邏輯（無硬編碼）

**核心承諾**：
- ✅ 新增 Phase A 節點時，DRYRUN 自動參考（從 pipeline.json 讀取 metrics 定義）
- ✅ 新增 Phase B 節點時，DRYRUN 自動生成規格（從 pipeline.json 讀取 spec_rules）
- ✅ 新增節點時無需改代碼，只需修改 `pipeline.json` + 創建三件套模板
...
1. **Step 1：動態讀取 pipeline.json**
   - 從 `templates/pipeline.json` 讀取完整的 metrics 和 spec_rules 定義
   - 確認所有定義格式正確（JSON 合法）

2. **Step 2：結構化讀取與量化計算**
   - 遍歷 `pipeline.json` 的 `metrics[]` 陣列（不再硬編碼）
   - 對每個 metric，根據 `source_step` 定位檔案，用 `grep_pattern` 提取計數
   - 提取 20 個量化指標（數量由 pipeline.json 決定，可動態擴展）
   - 無檔案輸出（結果保存在記憶體）

3. **Step 3：規格邏輯推導**
   - 遍歷 `pipeline.json` 的 `steps[]` 陣列，過濾出 Phase B steps（不再硬編碼）
   - 評估每個 step 的 `condition`（如 `client_type != none`）
   - 對激活的 step，直接讀取 `spec_rules` 欄位（不再硬編碼推導）
   - 每個規格含三部分：
     - `quantitative_specs`：數值型門檻（endpoint_count、table_count、section_count 等，由 spec_rules 定義）
     - `content_mapping`：內容涵蓋規則（entity 涵蓋、US 追蹤等，由 spec_rules 定義）
     - `cross_file_validation`：跨檔案一致性檢查（entity parity、endpoint mapping 等，由 spec_rules 定義）

4. **Step 4：驗證完整性**
   - 確認所有激活的 step 都有完整的 spec_rules
   - 確認沒有遺漏

5. **Step 5：規格存儲與清單生成**
   - 將所有規格嵌入 `.gendoc-state-*.json` 的 `step_specifications` 欄位
   - 輸出 `docs/MANIFEST.md`（量化基線 + 規格摘要 + review.sh 使用方式）

#### Output

- `.gendoc-state-*.json`（含 `quantitative_metrics` + `step_specifications`）
- `docs/MANIFEST.md`（人類可讀的量化基線與工具說明）

**不輸出任何 Review 工具或臨時檔案到目標項目中**

---

#### 新增節點時的工作流

**情景 1：新增 Phase A 節點（如 REVIEW.md）**

```
1. 編輯 templates/pipeline.json
   ├─ 在 steps[] 中新增 REVIEW step
   └─ 在 metrics[] 新增相關指標定義
       └─ {"id": "review_item_count", "source_step": "REVIEW", "grep_pattern": "^- \[ \]", "fallback": 0}

2. 創建三件套模板
   ├─ templates/REVIEW.md（結構）
   ├─ templates/REVIEW.gen.md（Quality Gate）
   └─ templates/REVIEW.review.md（審查標準）

3. DRYRUN 執行
   └─ 自動感知新 metric → 提取 review_item_count
   └─ 新 metric 自動計入量化基線
   └─ 無需改代碼
```
