...
| 欄位 | 內容 |
|------|------|
| **需求來源** | 架構增強（2026-05-02）：使用者提出在 pipeline 中加入 dryrun step，EDD 之後生成「預估清單」，讓後續每個 step 的 review 有機械式程式驗證作為補充，並整合 anti-fake（防假內容）、gate（完成門檻）機制，gate findings 與 review findings 統一受 gendoc-config 的 max_rounds / stop_condition 約束。 |
| **問題描述** | 目前 gendoc pipeline 的每個 step 只有 AI review → fix loop，存在以下結構性缺陷：**(1) 無機械式完整性驗證**：review agent 是語意判斷，可接受「API.md 有 §Endpoints 章節但只寫了 2 個 endpoints 而設計有 30 個」，因為它無法從 upstream docs 算出精確數量；目前沒有任何機制在生成後機械式地計算：endpoints 數量、DB table 數量、test case 數量、section 數量，並與 upstream spec 對照確認；**(2) 無 anti-fake 機制**：AI 生成的 .md 文件可能出現：殘留 `{{PLACEHOLDER}}` 未替換、段落重複複製（同一段文字出現 2+ 次）、章節只有標題沒有內容（< 30 words）、假資料（entity 名稱全是 FooEntity / BarModel 等無意義名稱）；review agent 可能對這些問題評分為 LOW 或忽略，因為它沒有精確的「完成標準」；**(3) 無預測性完整性基線**：pipeline 執行前沒有任何文件說明「此次執行預計生成哪些文件、每個文件應有哪些結構」，導致：執行到 D16-ALIGN 時才發現某個中間文件結構錯誤；debug 時難以判斷問題出在 gen agent 還是 review agent；無法提前知道哪些 step 是「條件觸發」且本次會被 skip；**(4) Review 品質依賴 AI prompt，缺乏量化錨點**：`ARCH.gen.md` 生成規則說「§3 Tech Stack table 必須列出所有元件」，但 review.gen.md 的對應 check 沒有量化：多少個 = 完整？目前靠 AI 主觀判斷「足夠完整」，不同次執行品質不一致。**(5) 參考：gencode 的 Recon phase 已解決類似問題**：`~/.claude/skills/gencode/templates/pipeline.json` 把 `recon-dryrun` 設為 pipeline 第一個 phase，在任何代碼生成前先讀取所有 upstream docs，為每個後續 sub-step 生成 `.gencode-rules/<sub-step-id>-rules.json`（artifacts + anti_fake_rules），之後每個 phase 的 gate script 機械式驗證規則文件；`rule-gate.mjs` 是純程式驗證，不依賴 AI 判斷。 |
| **專家評估** | **需求有效，設計方向正確，但 gendoc 需要在 3 個關鍵點上偏離 gencode 的實作方式。** **(A) 位置：不是「EDD 之後」，而是「ARCH review 完成後」**：使用者直覺正確（越早鎖定基線越好），但 EDD 只能提供 domain model 和服務邊界，**技術棧（決定哪些 conditional steps 要執行）在 ARCH.md §3 才確認**。例如：`HAS_FRONTEND: true/false`、`LANG: Java/Node/Python`（決定 BUILD_TOOL 和 Agent Pod image）、`HAS_AUDIO: true/false`（決定 AUDIO step 是否在 manifest 中出現）。若 DRYRUN 在 EDD 後、ARCH 前執行，rule files 只能基於不完整資訊生成，mandatory step 清單會有缺漏。結論：**DRYRUN 放在 ARCH-review-loop 之後，API 之前**，此時 EDD + PRD + ARCH 三個最重要的 upstream docs 均已完成 review，資訊最完整。**(B) Rule file 格式：針對 .md 文件的量化規則**：gencode 的量化規則基於 panel count（UI）、table count（DB）、endpoint count（API）。gendoc 文件為純 Markdown，量化維度不同，需轉換：`API.md`：endpoint 數量 = EDD §4.5 component diagram 中 REST interface 數（grep `<<REST>>` 或 `HTTP` 關鍵字統計）；`SCHEMA.md`：table 數量 = EDD §4.5 classDiagram entity 數（entity 以 `class EntityName` 形式定義）；`test-plan.md`：tech layer 覆蓋數 = ARCH.md §3 Tech Stack table 的 layer 數（Presentation / Application / Domain / Infrastructure）；`RTM.md`：row 數 ≥ PRD.md 中 User Story 數（`## US-` 出現次數）；`BDD .feature`：scenario 數 ≥ User Story 數 × 0.8（每個 US 至少 0.8 個 scenario）。這些計數均可 grep 實現，不需要 AI。**(C) Gate 與 Review 的整合方式**：gencode 的 gate 是 phase-level（一個 phase 的所有 sub-steps 完成後跑一次 gate）。gendoc 是 step-level 線性 pipeline，更適合 **step-level gate**：每個 step 完成後，gate 讀取 `.gendoc-rules/<step-id>-rules.json`，輸出 mechanical findings（MECHANICAL 類型，等同 CRITICAL 級別）。Mechanical findings 注入 review findings 清單，統一進入 review→fix loop，受 `max_rounds` / `stop_condition` 約束。AI review agent 看到 mechanical findings 時，必須優先處理再進行語意審查。這樣做的好處：(a) 不需要改變現有 review→fix loop 架構；(b) MECHANICAL findings 有確定性（grep 可驗證），fix agent 知道如何精確修復；(c) gate 重跑有確定性結果（不像 AI review 可能每次評分不同）。**(D) Anti-fake 規則設計**：對 gendoc .md 文件，最有效的 anti-fake 規則是：`no_bare_placeholder`（正則 `\{\{[A-Z_]+\}\}` 不得出現）；`min_section_words`（每個 `##` 或 `###` 章節的 body 至少 30 words，純標題章節視為 fake）；`no_duplicate_paragraphs`（相同 150 字以上的段落不得在同文件出現 2 次，防止 copy-paste 填充）；`required_keywords_per_section`（API.md §Endpoints 必須出現 GET/POST/PUT/DELETE/PATCH 至少 3 種；SCHEMA.md §Tables 必須出現 PRIMARY KEY 或 FK 或 INDEX）；`no_trivial_entity_names`（entity 名稱不得全是 FooXxx / BarYyy / SampleZzz，用正則 `(Foo|Bar|Baz|Sample|Test)[A-Z]` 出現 ≥ 3 次 = anti-fake 觸發）。**(E) Manifest.md 的必要性**：`docs/MANIFEST.md`（人類可讀）和 `.gendoc-rules/*.json`（機械可驗證）應同時生成。MANIFEST.md 是給開發者和 AI review agent 的「合約書」，明確說明本次 pipeline 執行後應有哪些文件、每個文件的最低完整性標準。在 `/gendoc-auto` 開始時若 MANIFEST.md 已存在，可以跳過 DRYRUN 重新生成（節省一個 step 執行時間）。 |
| **建議怎麼改** | **架構設計（4 個組成部分）**：**(Part 1) 新增 pipeline step `DRYRUN`**，位置：ARCH 之後、API 之前（現有 pipeline 中為第 9 位，即 ARCH-review-loop 完成後）。Step 定義：`{"id": "DRYRUN", "name": "Pipeline Dryrun & Rule Generation", "phase": "設計層-規劃", "condition": "always", "type": "planning", "gen_skill": "gendoc-gen-dryrun", "output": ["docs/MANIFEST.md", ".gendoc-rules/*.json"], "no_review_loop": true}`，注意：DRYRUN step 本身不跑 review→fix loop（它不是文件，是規則文件），完成即進下一步；**(Part 2) 新建三件套（DRYRUN 不需要 review.md，只需兩件）**：`templates/DRYRUN.md`（MANIFEST.md 的骨架結構，說明每個 step 的預計輸出）；`templates/DRYRUN.gen.md`（DRYRUN step 生成規則，包含所有量化規則的計算邏輯）。Rule file 格式（`.gendoc-rules/<step-id>-rules.json`）：`{"step_id": "API", "step_name": "API Design Document", "output_files": [{"path": "docs/API.md", "must_exist": true, "must_not_be_empty": true, "min_h2_sections": 6, "min_endpoint_count": <從 EDD 計算的實際數>, "content_rules": {"required_sections": ["API Overview", "Authentication", "Endpoints", "Error Codes"], "no_bare_placeholders": true, "min_words_per_section": 30}}], "anti_fake_rules": [{"type": "no_placeholder_strings", "pattern": "\\{\\{[A-Z_]+\\}\\}"}, {"type": "no_duplicate_paragraphs", "min_char_length": 150}, {"type": "min_section_words", "min_words": 30}, {"type": "required_keywords", "section": "Endpoints", "keywords": ["GET", "POST", "PUT", "DELETE"]}]}`；**(Part 3) 新建 gate checker script**：`tools/bin/gate-check.sh`（每個 step 執行後由 gendoc-flow 自動呼叫）：讀取 `.gendoc-rules/<step-id>-rules.json`；對每個 `output_files` 執行：(a) 存在性檢查（`test -f`）；(b) 非空檢查（`wc -c`）；(c) 章節數計數（`grep -c '^## '`）；(d) endpoint 數計數（`grep -cE '^\| *(GET|POST|PUT|DELETE|PATCH)'`）；(e) 無裸 placeholder 檢查（`grep -cE '\{\{[A-Z_]+\}\}'`）；對每個 `anti_fake_rules` 執行對應的 grep 或 awk 邏輯；輸出格式為 JSON：`[{"severity": "MECHANICAL", "rule_id": "AF-01", "check": "no_bare_placeholders", "file": "docs/API.md", "detail": "發現 3 個未替換的 {{PLACEHOLDER}}：{{API_VERSION}}, {{BASE_URL}}, {{AUTH_TOKEN}}", "fix_hint": "替換所有 {{}} 為實際值"}]`；**(Part 4) 修改 `gendoc-flow/SKILL.md`**，在每個 step 的 review→fix loop 的 **第一個 round 之前**，先執行 gate-check.sh：`_GATE_FINDINGS=$(bash tools/bin/gate-check.sh <step-id> 2>/dev/null || echo "[]")`；`_GATE_COUNT=$(echo "$_GATE_FINDINGS" \| python3 -c "import json,sys; print(len(json.load(sys.stdin)))")`；若 `_GATE_COUNT > 0`：將 MECHANICAL findings 注入 review agent prompt（加在「本輪審查額外 mechanical findings：」章節），review agent 的 fix guidance 應以修復 MECHANICAL findings 為最高優先；修改 round summary：顯示 `MECHANICAL=N  AI-CRITICAL=N  AI-HIGH=N`；gate-check.sh 在每個 fix round 後**重跑一次**，確認 MECHANICAL findings 是否已消除（MECHANICAL findings 不依賴 AI 重評，是確定性驗證）；**(量化規則計算邏輯，由 DRYRUN gen agent 執行)**：`EDD_ENTITY_COUNT=$(grep -c '^\s*class ' docs/EDD.md 2>/dev/null || echo "5")`；`EDD_REST_COUNT=$(grep -cE '(<<REST>>|<<API>>|HTTP\s*(GET|POST))' docs/EDD.md 2>/dev/null || echo "10")`；`PRD_US_COUNT=$(grep -c '^## US-' docs/PRD.md 2>/dev/null || echo "10")`；`ARCH_LAYER_COUNT=$(grep -c '^\| [A-Za-z]' docs/ARCH.md 2>/dev/null || echo "4")`；這些數值寫入各 step 的 rule file，成為不可更改的完成標準；**(MANIFEST.md 結構)**：`# Pipeline Execution Manifest`；Document Control 表（生成日期、pipeline 版本、EDD entity count、PRD US count、ARCH layer count）；Mandatory Steps 清單（本次 pipeline 必須執行的 steps + expected output 路徑）；Conditional Steps 清單（HAS_FRONTEND=true 才執行 FRONTEND step 等）；Per-Step 完整性標準（表格：step | output files | min_sections | quantitative_rules | anti_fake_rules）。 |
| **目標檔案** | 新建：`templates/DRYRUN.md`（MANIFEST.md 骨架）；`templates/DRYRUN.gen.md`（量化規則計算 + rule file 生成邏輯）；`skills/gendoc-gen-dryrun/SKILL.md`（DRYRUN step gen agent，讀 EDD/PRD/ARCH → 計算量化數值 → 生成所有 rule files + MANIFEST.md）；`tools/bin/gate-check.sh`（機械式 gate checker，純 bash + grep/awk，不依賴 AI）；修改：`templates/pipeline.json`（新增 DRYRUN step，位置 = ARCH-review-loop 之後、API 之前）；`skills/gendoc-flow/SKILL.md`（在每個 step review-loop 第一個 round 前加入 gate_check 邏輯，共 ~30 行新增） |
