...
- DRYRUN 后的 step 所有步驟的 gate-check 依賴 `.gendoc-rules/*.json`
- 未跑 DRYRUN 直接補 DRYRUN 后的 step = 沒有品質閘門
- repair 必須感知這個邊界，並引導使用者先完成 DRYRUN 前的 step → DRYRUN → DRYRUN 后的 step
...
## UML Class Diagram 設計原則（每個 AI session 必讀）

### 原則一：`gendoc-gen-diagrams` 是唯一 UML 生成來源

- Pipeline 中所有 UML 圖（含 class diagrams，共 9 種）**只由 `gendoc-gen-diagrams`（step D07b-UML）生成**
- 生成規則 inline 在 `skills/gendoc-gen-diagrams/SKILL.md §2.2-2.4`（Server）和 `§2B-2~2B-4`（Frontend）
- 需要補跑或重做，答案只有一個：**直接呼叫 `gendoc-gen-diagrams` skill**
- 任何 review template 的 fix hint 若涉及 UML，只能指向 `gendoc-gen-diagrams`

### 原則二：其他文件的 inline 圖是另一回事

- EDD.md、ARCH.md 等文件本身的 Mermaid 程式碼塊是該文件的內容
- 這與 `docs/diagrams/` 下的 pipeline 輸出是兩件不同的事，不得混淆

### 原則三：`/gendoc UML-CLASS` 是實驗性小工具，完全隔離

- **不加入 pipeline.json**
- **不在任何生產 skill 中提及**（gendoc-gen-diagrams、gendoc-flow、gendoc-repair 等）
- **不在任何生產 template 中提及**（EDD.review.md、EDD.gen.md、PRD.md 等）
- 只存在於自己的三個檔案：`templates/UML-CLASS.md`、`templates/UML-CLASS.gen.md`、`templates/UML-CLASS.review.md`
- 唯一入口：`SKILL.md` alias `uml-class → UML-CLASS`

### 已確認的生產狀態（2026-05-03）

- `templates/UML-CLASS-GUIDE.md`：已刪除（早期手動期遺留，pipeline 從未讀取）
- `gendoc-gen-diagrams/SKILL.md §2.9`：已追加 3 個 Class Diagram 品質閘門
  1. 全部 6 種關聯類型各出現 ≥ 1 次
  2. 每張圖 ≥ 6 個 class（含 ≥ 1 `<<interface>>`）
  3. 每張圖末端附「技術說明」＋「白話說明」
