...
│   ├── dryrun_core/                # DRYRUN parameter extraction SSOT + tests
│   │   └── dryrun_core.py          # ← SSOT; `setup deploy` copies to tools/bin/ (not git-tracked)
│   ├── guard/                      # gendoc-guard simulators & rule tests
│   └── bin/                        # Runtime executables (shell scripts = SSOT; .py = deploy artifacts)
│       ├── gen_html.py             # deploy artifact — copied from tools/gen_html/ by setup deploy
│       ├── dryrun_core.py          # deploy artifact — copied from tools/dryrun_core/ by setup deploy
│       ├── review.sh / review_integration.sh  # SSOT — DRYRUN output validation
│       ├── get-upstream.sh         # SSOT — upstream file/section reader (always called via runtime GENDOC_DIR)
│       ├── gendoc-guard-blocker.py      # SSOT — PreToolUse SECS whitelist hook (exit 2 = block)
│       ├── gendoc-guard-history.py      # SSOT — PostToolUse execution history hook
│       ├── gendoc-guard-session-start.py # SSOT — SessionStart resume injection hook
│       └── gendoc-guard-stop.py         # SSOT — Stop hook — queue + macOS notification + dodge detection
