...
若 EDD §10.3 或 §4.5.3 已有程式碼，直接提取；若無，依 EDD §3（DDD Aggregate 設計）取一個代表性場景推斷。
...
3. **最低張數要求：≥ 3 張**，必須涵蓋：
   - Happy Path（最常見主流程，1 張）
   - Error Path（主要錯誤/異常處理，1 張）
   - 次要業務流程（至少 1 張）
4. 若 API.md 有核心 Endpoint（CRUD 主操作）但 EDD 無對應 Sequence Diagram，依 API endpoint 說明推斷並生成
5. 若已提取的 Sequence 張數 < 3，**必須補充**至 ≥ 3 張（從 API.md 端點列表推斷）
...
若 EDD §10.5 或 §4.5.5 已有程式碼，直接提取；若無，依 EDD §3（DDD Context Map）或 EDD Mermaid 架構圖章節的元件交互圖推斷主要物件協作關係。
...
║  [4] Sequence Diagram       → sequence-*.md（≥3張）    ✓/✗ ║
