...
- 非同步操作：`par [async: 說明非同步原因]` 包裹（**見下方 par 語法規則**）
- 重試邏輯：`loop [retry: 最多 N 次，間隔 Xms]` 包裹
- 資料庫事務：明確標注 `DB->>DB: BEGIN TRANSACTION` / `COMMIT` / `ROLLBACK ON ERROR`

**Mermaid v11 `par` 語法規則（違反即破圖）**：
- `par` block **必須**至少含一個 `and` 分隔子句；缺少 `and` 導致 mermaid v11 解析失敗（"Syntax error in text"）
- 正確格式（雙分支，含 `and`）：
  ```
  par async: 操作A
      A->>B: 非同步請求A
  ...
  ```
- 若並行操作只有**一個**分支（無第二條並行線），**禁止**使用 `par`；改用 `Note over A: async: 說明` 替代
...
## Step 2.9-V：Mermaid v11 語法驗證（Fix Loop，最多 3 輪）

**目的**：掃描所有 `docs/diagrams/*.md` 中的 mermaid 程式碼塊，偵測並自動修復已知的 v11 語法錯誤。
**執行時機**：Step 2.9 完整性驗證通過後、Step 2B 之前，必須執行。

```bash
_DIAG_DIR="$(pwd)/docs/diagrams"
_MAX_V_ROUNDS=3
_V_ROUND=0
_V_ISSUES=999

while [[ $_V_ROUND -lt $_MAX_V_ROUNDS && $_V_ISSUES -gt 0 ]]; do
  _V_ROUND=$((_V_ROUND + 1))
  echo "[diag-syntax] Round $_V_ROUND/$_MAX_V_ROUNDS"

  _V_REPORT=$(python3 - "$_DIAG_DIR" <<'PY'
import re, sys
...
PY
  ...
  _V_ISSUES=$(echo "$_V_REPORT" | grep "^ISSUE_COUNT:" | cut -d: -f2)
  echo "[diag-syntax] 發現問題：$_V_ISSUES"

  if [[ "$_V_ISSUES" -gt 0 ]]; then
    echo "$_V_REPORT" | grep "^ISSUE:"

    # 自動修復：移除缺少 and 的 par/end 包裝，保留內容
    python3 - "$_DIAG_DIR" <<'FIXPY'
import re, sys
from pathlib import Path
    ...
    while i < len(block_lines):
        ...
        if re.match(r'^par\b', s):
            ...
            while j < len(block_lines):
                ...
                j += 1
            if not found_and:
                # Skip par line; keep content; skip matching end
                i += 1; depth = 1
                while i < len(block_lines):
...
if [[ "$_V_ISSUES" -gt 0 ]]; then
  echo "⚠️  [diag-syntax] 仍有 $_V_ISSUES 個語法問題（自動修復 $_MAX_V_ROUNDS 輪後殘留）"
  echo "      如仍有 par_without_and 錯誤，請重新生成對應的 Sequence Diagram 檔案"
else
  echo "✅ [diag-syntax] 所有 Mermaid v11 語法驗證通過"
fi
```
