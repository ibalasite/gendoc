...
**[AI 指令]** 每次進入 Step 1 前，先重新讀取 state file，顯示最新狀態：

```bash
if [[ -n "$_STATE" && -f "$_STATE" ]]; then
  _STEP=$(python3 -c "import json; d=json.load(open('$_STATE')); print(d.get('start_step','未設定'))" 2>/dev/null)
  _STRATEGY=$(python3 -c "import json; d=json.load(open('$_STATE')); print(d.get('review_strategy') or '未設定')" 2>/dev/null)
  _CT=$(python3 -c "import json; d=json.load(open('$_STATE')); print(d.get('client_type') or '未設定')" 2>/dev/null)
  _HAB=$(python3 -c "import json; d=json.load(open('$_STATE')); v=d.get('has_admin_backend'); print('未設定' if v is None else str(v))" 2>/dev/null)
  echo ""
  echo "╔══════════════════════════════════════════════╗"
  echo "║  目前設定                                      ║"
  echo "╠══════════════════════════════════════════════╣"
...
```
question: "你想修改什麼？（可多次修改，完成後選「確認完成」）"
options:
  - "重設流程進度（重新跑全部 / 從某步開始）"
  - "修改審查強度"
  - "修改專案設定（client_type / Admin 後台 / 清除設定）"
  - "✅ 確認完成，儲存離開"
```
...
### 第一層選「修改審查強度」或「確認完成」→ 直接進入 Step 2

---

## Step 2：依選擇執行（每個分支完成後回 Step 1）

### 選「重設流程進度」→「重新跑全部流程」

→ 設 `_NEW_STEP = "0"`，繼續 Step 3（審查強度）→ Step 4 寫入 → **回 Step 1**
...
→ 設 `_NEW_STEP = <使用者選擇文字中 `:` 前的 ID 部分>`，繼續 Step 3 → Step 4 → **回 Step 1**

---

### 選「修改審查強度」

→ 設 `_NEW_STEP = $_STEP`（維持現有進度），繼續 Step 3（審查強度）→ Step 4 寫入 → **回 Step 1**
...
→ 直接跳到 Step 4 寫入（跳過 Step 3）→ **回 Step 1**
...
→ 清除後直接結束（若要重新設定，請再執行 /gendoc-config）
...
## Step 4c：必填欄位檢查（儲存前強制補問）

**[AI 指令]** 使用者選「確認完成」時，在真正離開前，依序執行以下三項檢查。每項「未設定」就強制問，已有值則跳過：

```bash
python3 -c "
import json
d = json.load(open('$_STATE'))
missing = []
if not d.get('client_type'):               missing.append('client_type')
if d.get('has_admin_backend') is None:     missing.append('has_admin_backend')
if not d.get('review_strategy'):           missing.append('review_strategy')
print(','.join(missing))
"
```

- 若缺 **client_type** → 強制問（同情境 B 的 client_type 問題）→ 寫入 state
- 若缺 **has_admin_backend** → 強制問（同「修改專案設定」→「has_admin_backend」問題）→ 寫入 state
- 若缺 **review_strategy** → 強制進入 Step 3（審查強度）→ Step 4 寫入

所有欄位都有值後 → 進入 Step 5 輸出摘要，結束。
