...
Scope:   僅負責 IDEA + BRD；其餘文件由 gendoc-flow 接手
```
...
if [[ -f "$_PROJECT_DIR/templates/pipeline.yml" ]] || [[ -f "$_PROJECT_DIR/templates/pipeline.json" ]]; then
...
echo "[Step 1.8] 建議 client_type：${_SUGGEST_CT} / has_admin_backend：${_SUGGEST_ADMIN}"
```
...
print('[Step 1.8] auto: client_type='+d['client_type']+' / has_admin_backend='+str(d['has_admin_backend']))
PYEOF
fi
```

### 互動模式：client_type 確認（全流水線唯一互動點）

**若 `_SKIP_1_8=false`**：

**步驟 A — 檢查 state 是否已有 client_type 和 has_admin_backend**

```bash
_CT_OK=$(python3 -c "import json; d=json.load(open('${_STATE_FILE}')); print('true' if d.get('client_type') else 'false')" 2>/dev/null || echo "false")
_ADMIN_OK=$(python3 -c "import json; d=json.load(open('${_STATE_FILE}')); print('true' if 'has_admin_backend' in d else 'false')" 2>/dev/null || echo "false")
```

若 `_CT_OK=false` 或 `_ADMIN_OK=false`：透過 **Skill tool** 呼叫 `gendoc-config`（統一設定入口），讓使用者填入 client_type 和 has_admin_backend 並寫入 state，完成後繼續。

若兩者均已有值，則跳到步驟 B。

**步驟 B — 確認 client_type（僅在 state 無 client_type 時詢問；若已由 gendoc-config 填入則跳過）**：

若仍需確認 client_type，用 `AskUserQuestion`：
...
> `has_admin_backend` 由系統依 `client_type` 自動推斷（非 api-only 預設 true）或由 gendoc-config 設定。
> 容量規劃（DAU / PCU）由 PM Expert 在 Step 3 web 研究後自動推估，無需詢問使用者。
...
if not d.get('client_type'):
    d['client_type']        = '${_CONFIRMED_CT}'
    d['client_type_source'] = 'confirmed'   # P-13/P-14 不再覆寫
if 'has_admin_backend' not in d:
    d['has_admin_backend']  = '${_SUGGEST_ADMIN}' == 'true'
tmp=f+'.tmp'; open(tmp,'w').write(json.dumps(d,indent=2,ensure_ascii=False)); os.replace(tmp,f)
print('[Step 1.8] ✅ client_type='+d['client_type']+' / has_admin_backend='+str(d['has_admin_backend'])+' 已確認並鎖定')
PYEOF
...
### Step 3.1：PM Expert 容量推估（依研究結果自動推斷 DAU / PCU）

**[AI 指令]** 在 PM Expert 角色下，依 Step 3 的競品/市場研究結果，推估本專案啟動期預期流量：
...
# PM Expert 從 _RESEARCH_SUMMARY 競品規模、行業基準推算
# 填入推估值（AI 依研究結果替換以下佔位符）：
d['q4_dau']            = '<推估 DAU，如 "50,000/day">'
d['q4_peak_ccu']       = '<推估同時在線峰值，如 "3,000 人">'
d['q4_estimate_basis'] = '<推估依據，如 "競品 A 月活 100 萬，DAU/MAU 比約 5%">'
tmp = f + '.tmp'
open(tmp, 'w', encoding='utf-8').write(json.dumps(d, indent=2, ensure_ascii=False))
os.replace(tmp, f)
print('[PM Expert 容量] DAU=' + d['q4_dau'] + ' / PCU=' + d['q4_peak_ccu'])
PYEOF
```

> 注意：q4_dau、q4_peak_ccu、q4_estimate_basis **由 PM Expert 自動推斷，不詢問使用者**。
> 這些數值影響 EDD 容量規劃、ARCH HPA 閾值，不影響架構選型（架構均採 HA 設計）。
...
if [[ ",${_COMPLETED_NOW}," == *",IDEA,"* ]]; then
  if [[ "$_IDEA_REVIEWED" == "True" ]]; then
    echo "[Skip P-01] IDEA 已完成且 review 通過，跳過 IDEA 生成與 review"
    _SKIP_IDEA_GEN="true"
  else
    echo "[Resume P-01] IDEA 已生成但 review 未通過，跳過生成，直接進入 review"
    _SKIP_IDEA_GEN="true"
...
if 'IDEA' not in d.get('completed_steps',[]):
    d.setdefault('completed_steps',[]).append('IDEA')
json.dump(d, open(f,'w'), ensure_ascii=False, indent=2)
"
...
**以 Agent tool 建立 subagent 驅動整個 IDEA Review Loop**（不得在主 Claude 直接執行 loop，否則 context 將爆滿導致 Step 8 無法自動觸發）。
...
echo "[IDEA Review Loop] 策略：${_REVIEW_STRATEGY}，最多 ${_MAX_ROUNDS} 輪"
```

主 Claude 派送以下 Agent（填入實際值：`$_PROJECT_DIR` / `$_TEMPLATE_DIR` / `$_MAX_ROUNDS` / `$_REVIEW_STRATEGY`）：

> **任務**：對 docs/IDEA.md 執行完整的 Review → Fix → Commit loop，直到通過或達上限。
> **專案目錄**：{_PROJECT_DIR}（先 cd 進去）
> **Templates 目錄**：{_TEMPLATE_DIR}
> **max_rounds**：{_MAX_ROUNDS}　**review_strategy**：{_REVIEW_STRATEGY}
>
> **預備**：
> 1. cd {_PROJECT_DIR}
> 2. 讀取 {_TEMPLATE_DIR}/IDEA.review.md（獲取所有 reviewer-roles + review items）
> 3. 讀取 docs/IDEA.md
>
> **對每個 round（1 到 {_MAX_ROUNDS}）**：
> a. **Review**：扮演 IDEA.review.md 定義的所有審查角色，逐項執行 Check（引用文件具體§章節）
> b. **判斷終止**：
>    - finding_total=0 → PASSED
>    - strategy=rapid 且 round≥3 → MAX_ROUNDS
>    - strategy=tiered 且 round≥6 且 CRITICAL+HIGH+MEDIUM=0 → PASSED
>    - round≥max_rounds → MAX_ROUNDS
> c. **Fix**（finding>0 時必執行）：精準修復 docs/IDEA.md
>    - CRITICAL+HIGH 必修；MEDIUM+LOW 盡力
>    - 只修改 finding 指出的具體位置，保持其他部分不變
> d. **Round Summary**（每輪必輸出）：
>    ┌─── IDEA Review Round N/{max_rounds} ──────────────────────────┐
>    │  Review：CRITICAL=N HIGH=N MEDIUM=N LOW=N  Total=N
>    │  Fix：   修復 N 個 / 殘留 N 個
>    │  本輪狀態：✅ PASS | ⚠️  MAX | 🔄 CONT   {terminate_reason}
>    └────────────────────────────────────────────────────────────────────┘
> e. **Commit**：
>    - finding=0：`git add docs/IDEA.md && git commit -m "docs(gendoc-auto)[IDEA]: review-r{N} — PASS (0 findings)"`
>    - finding>0：`git add docs/IDEA.md && git commit -m "docs(gendoc-auto)[IDEA]: review-r{N} — fix {fixed}/{total} findings"`
> f. 若 terminate=True → break
>
...
>   passed: true|false
>   terminate_reason: "PASSED — finding=0 | MAX_ROUNDS=N 已達"
>   summary: "N 輪完成，最終 finding=N（PASSED/MAX_ROUNDS）"

等 Agent 回傳 REVIEW_LOOP_RESULT 後，主 Claude 更新 state：
...
if [[ ",${_COMPLETED_NOW}," == *",BRD,"* ]]; then
  if [[ "$_BRD_REVIEWED" == "True" ]]; then
    echo "[Skip P-01] BRD 已完成且 review 通過，跳過 BRD 生成與 review"
    _SKIP_BRD_GEN="true"
  else
    echo "[Resume P-01] BRD 已生成但 review 未通過，跳過生成，直接進入 review"
    _SKIP_BRD_GEN="true"
...
if 'BRD' not in d.get('completed_steps',[]):
    d.setdefault('completed_steps',[]).append('BRD')
json.dump(d, open(f,'w'), ensure_ascii=False, indent=2)
"
...
**以 Agent tool 建立 subagent 驅動整個 BRD Review Loop**（不得在主 Claude 直接執行 loop，否則 context 將爆滿導致 Step 8 無法自動觸發）。
...
" 2>/dev/null || echo "standard")
echo "[BRD Review Loop] 策略：${_REVIEW_STRATEGY}，最多 ${_MAX_ROUNDS} 輪"
```

主 Claude 派送以下 Agent（填入實際值：`$_PROJECT_DIR` / `$_TEMPLATE_DIR` / `$_MAX_ROUNDS` / `$_REVIEW_STRATEGY`）：

> **任務**：對 docs/BRD.md 執行完整的 Review → Fix → Commit loop，直到通過或達上限。
> **專案目錄**：{_PROJECT_DIR}（先 cd 進去）
> **Templates 目錄**：{_TEMPLATE_DIR}
> **max_rounds**：{_MAX_ROUNDS}　**review_strategy**：{_REVIEW_STRATEGY}
>
> **預備**：
> 1. cd {_PROJECT_DIR}
> 2. 讀取 {_TEMPLATE_DIR}/BRD.review.md（獲取所有 reviewer-roles + review items）
> 3. 讀取 docs/IDEA.md（上游文件）
> 4. 讀取 docs/BRD.md
>
> **對每個 round（1 到 {_MAX_ROUNDS}）**：
> a. **Review**：扮演 BRD.review.md 定義的所有審查角色，逐項執行 Check（引用文件具體§章節）
> b. **判斷終止**：
>    - finding_total=0 → PASSED
>    - strategy=rapid 且 round≥3 → MAX_ROUNDS
>    - strategy=tiered 且 round≥6 且 CRITICAL+HIGH+MEDIUM=0 → PASSED
>    - round≥max_rounds → MAX_ROUNDS
> c. **Fix**（finding>0 時必執行）：精準修復 docs/BRD.md
>    - CRITICAL+HIGH 必修；MEDIUM+LOW 盡力
>    - 只修改 finding 指出的具體位置，保持其他部分不變
> d. **Round Summary**（每輪必輸出）：
>    ┌─── BRD Review Round N/{max_rounds} ───────────────────────────┐
>    │  Review：CRITICAL=N HIGH=N MEDIUM=N LOW=N  Total=N
>    │  Fix：   修復 N 個 / 殘留 N 個
>    │  本輪狀態：✅ PASS | ⚠️  MAX | 🔄 CONT   {terminate_reason}
>    └─────────────────────────────────────────────────────────────────────┘
> e. **Commit**：
>    - finding=0：`git add docs/BRD.md && git commit -m "docs(gendoc-auto)[BRD]: review-r{N} — PASS (0 findings)"`
>    - finding>0：`git add docs/BRD.md && git commit -m "docs(gendoc-auto)[BRD]: review-r{N} — fix {fixed}/{total} findings"`
> f. 若 terminate=True → break
>
> **完成後必須輸出**（格式嚴格，主 Claude 解析此區塊）：
> REVIEW_LOOP_RESULT:
...
# 標記 IDEA + BRD 已完成，讓 gendoc-flow 從 PRD 開始
completed = d.get('completed_steps', [])
for step_id in ['IDEA', 'BRD', 'handoff']:
...
echo ""
echo "╔══════════════════════════════════════════════════════════╗"
echo "║  ✅ IDEA.md + BRD.md 通過 Review，正在啟動 gendoc-flow   ║"
echo "╠══════════════════════════════════════════════════════════╣"
echo "║  接下來將從 PRD 自動繼續整條文件流水線。                 ║"
echo "╚══════════════════════════════════════════════════════════╝"
```

**立即用 Skill 工具呼叫 `gendoc-flow`**（不得只輸出文字，必須真正觸發工具）。

**若 Skill 工具觸發失敗**，輸出：

```bash
echo "╔══════════════════════════════════════════════════════════╗"
echo "║  [P-08] 請立即輸入：/gendoc-flow                         ║"
echo "╚══════════════════════════════════════════════════════════╝"
```
