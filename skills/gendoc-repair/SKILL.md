---
name: gendoc-repair
description: DRYRUN-aware backfill — 把不完整的目標專案補到與 gendoc-auto + gendoc-flow 從頭執行完全一致的狀態。分三區 diff（DRYRUN 前的 step / DRYRUN Gate / DRYRUN 后的 step）、DRYRUN 三態偵測、rules.json 品質驗證、兩階段補跑模式。
version: 2.0.0
allowed-tools:
  - Read
  - Write
...
# gendoc-repair — DRYRUN-Aware Backfill
...
## Step 1：Pipeline Diff 分析（DRYRUN-Aware）

**[AI 指令]** 用 Python 計算差異，分三區輸出：DRYRUN 前的 step、DRYRUN Gate、DRYRUN 后的 step：
...
# ── Header ──
print(f"\n{'='*70}")
print(f"  Pipeline Diff 報告（DRYRUN-Aware）")
...
║  /gendoc-repair 完成                                              ║
╠══════════════════════════════════════════════════════════════════╣
║  ✅ DRYRUN 前的 step 完整 · DRYRUN OK · DRYRUN 后的 step 完整 · 品質驗證通過 ║
...
if not _IDEA_OK:
    ...
    if not _COMPLETED:
        ...
        print("       請先執行 /gendoc-auto 或 /gendoc-flow BRD PRD ... 完成 DRYRUN 前的基礎")
        exit(1)
    else:
...
# DRYRUN 必須排在 DRYRUN 前的 step 之後、DRYRUN 后的 step 之前（若在序列中）
