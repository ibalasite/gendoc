...
STEP_SEQUENCE 和 STEP_ORDER 由 pipeline.json 動態產生，不再硬編碼。
`templates/pipeline.json` 是唯一順序來源，新增/修改步驟只需改 pipeline.json。

**[AI 指令]** 需要 STEP_SEQUENCE / STEP_ORDER 時，執行以下腳本取得當前值：
...
if [[ -f "$_CWD/templates/pipeline.json" ]]; then
  ...
  echo "⚠️  找不到 pipeline.json"; exit 1
fi
...
for s in pipe.get("steps", []):
    ...
    special = s.get("special_skill", "")
    cond_str    = f"（{cond}）" if cond != "always" else ""
    special_str = f"（{special}，特殊步驟）" if special else ""
    seq[sid] = f"{stype} 生成{cond_str}{special_str}"
    order.append(sid)
print("STEP_SEQUENCE:", json.dumps(seq, ensure_ascii=False, indent=2))
print()
print("STEP_ORDER:", json.dumps(order, ensure_ascii=False))
PY
```
...
**定義：** pipeline.json 中 `special_skill` 欄位為空的所有步驟都適用 Review Loop。
特殊步驟（有 `special_skill`）走自己的 skill，不跑標準 review loop。

**[AI 指令]** 需要 Review Loop 適用清單時，動態計算：

```bash
python3 - "$_PIPELINE_FILE" <<'PY'
import json, sys
pipe = json.load(open(sys.argv[1], encoding="utf-8"))
review_steps = [s["id"] for s in pipe.get("steps", []) if not s.get("special_skill")]
print("Review Loop 適用 STEP（共 {} 個）：".format(len(review_steps)))
print("、".join(review_steps))
PY
```
...
所有 STEP 的 commit 訊息使用 `pipeline.json` 中定義的 `commit_prefix`，格式為：

```
<type>(gendoc)[D{NN}-TYPE]: <簡述>
```

`commit_prefix` 的唯一定義在 `templates/pipeline.json` 的每個 step 的 `commit_prefix` 欄位。

### type 對照

| STEP 類型 | type |
|-----------|------|
| 文件生成（PRD/EDD/ARCH/API/SCHEMA 等）| `docs` |
| BDD / Feature Files | `test` |
| 機器可讀規格（CONTRACTS）| `feat` |
| 對齊/診斷 | `docs` |
| HTML/Pages | `docs` |
