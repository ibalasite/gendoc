...
description: |
  ...
  1. OpenAPI 3.1 YAML（docs/openapi.yaml）
  2. JSON Schema（docs/contracts/schemas/*.json）
  3. Pact Consumer Contract（docs/contracts/pact/*.json）
  4. IaC 模板（infra/helm/values.yaml 或 docker-compose.yml + network-policy）
  5. Seed Code Skeleton（scaffold/src/）
  呼叫時機：gendoc-flow D18-CONTRACTS 步驟（所有文件完成後）；或用戶手動執行
allowed-tools:
  - Read
  - Write
  - Edit
...
mkdir -p docs/contracts/schemas docs/contracts/pact infra/k8s scaffold/src
```
...
**生成後用 Write 工具寫入** `docs/openapi.yaml`。
...
若 Agent 成功，顯示：
```
✅ OpenAPI: docs/openapi.yaml（{paths_count} paths, {schemas_count} schemas）
```
...
**每個 DTO 生成一個文件**，路徑：`docs/contracts/schemas/{ClassName}.schema.json`
...
**每個 consumer-provider 對生成一個文件**：`docs/contracts/pact/{Consumer}--{Provider}.json`
...
**輸出 2**: `infra/k8s/network-policy.yaml`
```yaml
...
**輸出路徑**：在 `scaffold/` 下重建 Class Inventory 的 src 路徑結構

**每個 class 生成對應骨架文件**（根據語言）：

### TypeScript 範例：
```typescript
// scaffold/src/domain/order/Order.ts
...
```typescript
// scaffold/tests/unit/domain/order/Order.test.ts
...
|----------|------|---------|--------|
| OpenAPI 3.1 YAML | [docs/openapi.yaml](openapi.yaml) | API 合約，可匯入 Postman / Swagger UI / code generator | docs/API.md + EDD.md §4.5.4 |
| JSON Schema | [docs/contracts/schemas/](contracts/schemas/) | DTO 型別定義，供 validator / IDE / code generator 使用 | EDD.md §4.5.2 |
| Pact Contracts | [docs/contracts/pact/](contracts/pact/) | Consumer-driven 合約測試規格 | EDD.md §4.5.4 |
| Helm Values | [infra/helm/values.yaml](../infra/helm/values.yaml) | Kubernetes 部署配置 | EDD.md §4.5.9 |
| Network Policy | [infra/k8s/network-policy.yaml](../infra/k8s/network-policy.yaml) | 網路存取控制規則 | EDD.md §4.5.9 Network Policy Table |
| Seed Code | [scaffold/src/](../scaffold/src/) | 骨架代碼（空實作）+ 測試骨架 | EDD.md §4.5.2 Class Inventory |
...
npx openapi-generator-cli generate -i docs/openapi.yaml -g typescript-axios -o src/generated/client
...
npx ajv validate -s docs/contracts/schemas/CreateOrderRequest.schema.json -d payload.json
```

### Pact Contract Testing
```bash
# Provider verification
npx pact-verifier --pact-urls docs/contracts/pact/*.json \
...
### Deploy with Helm
```bash
helm install {project-name} ./infra/helm -f infra/helm/values.yaml
```

### Scaffold
```bash
# Seed code is in scaffold/ — copy to src/ to begin implementation
...
[[ -f "docs/openapi.yaml" ]] && _FILES="$_FILES docs/openapi.yaml"
[[ -d "docs/contracts" ]] && _FILES="$_FILES docs/contracts/"
[[ -d "infra" ]] && _FILES="$_FILES infra/"
[[ -d "scaffold" ]] && _FILES="$_FILES scaffold/"
[[ -f "docs/CONTRACTS.md" ]] && _FILES="$_FILES docs/CONTRACTS.md"

if [[ -n "$_FILES" ]]; then
  git add $_FILES 2>/dev/null || true
  git commit -m "feat(gendoc)[D18-CONTRACTS]: generate machine-readable specs from docs

- OpenAPI 3.1 YAML: docs/openapi.yaml
- JSON Schema: docs/contracts/schemas/
- Pact contracts: docs/contracts/pact/
- IaC: infra/ (${_DEPLOY_TARGET})
- Seed code skeleton: scaffold/src/
...
╠══════════════════════════════════════════════════════════════════╣
║  OpenAPI:      docs/openapi.yaml（{N} paths, {N} schemas）       ║
║  JSON Schema:  docs/contracts/schemas/（{N} 個 DTO）              ║
║  Pact:         docs/contracts/pact/（{N} 個 consumer-provider）  ║
║  IaC:          infra/（{deploy_target}）                          ║
║  Seed Code:    scaffold/src/（{N} 個文件，{lang}）                ║
╠══════════════════════════════════════════════════════════════════╣
║  下一步：                                                         ║
║  1. npx @redocly/openapi-cli lint docs/openapi.yaml              ║
║  2. 複製 scaffold/src/ → 開始實作                                  ║
