...
description: |
  ...
  呼叫時機：gendoc-flow MOCK 步驟；或用戶手動執行。
...
if [[ "$_CLIENT_TYPE" == "api-only" ]]; then
  ...
  echo "STEP_COMPLETE: MOCK"
  exit 0
fi

# 確認上游文件存在
[[ ! -f "docs/API.md" ]] && echo "[ERROR] docs/API.md 不存在，請先完成 API 步驟" && exit 1
...
git commit -m "feat(gendoc)[MOCK]: generate FastAPI mock server
...
下一步：HTML — HTML 文件網站生成（含 mock server 連結）
```
